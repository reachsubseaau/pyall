...
def point2raster(outfilename, geo, pcd, resolution=1, bintype="mean", fill=False):
    ...
    # here is a version which runs woth non integer resolutions. we do this by multiplying the easting and northing by 100
    #take the point cloud array and transpose the xyz,xyz,xyz into xxx,yyy so we can bin them efficienctly without looping thru the data
    xy = pcd.T[:2]
    mnoriginal, mxoriginal = xy.min(axis=1), xy.max(axis=1)
 
    # convert to cm
    xy = np.multiply(xy, 100)
    #bin the xy data into buckets.  at present this is only integer based so 1m resolution is minimum
    xy = ((xy + (resolution*100) / 2) // (resolution * 100)).astype(int)
    #compute the range of the data 
    mn, mx = xy.min(axis=1), xy.max(axis=1)
    #compute the size of the data
    sz = mx + 1 - mn
    ...
    src= rasterio.open(
            ...
            transform=transform,
            nodata=NODATA,
            # compress = Compression.lzw.name,  # Use LZW compression
            compress = Compression.deflate.name,  # Use Deflate compression (lossless)
            # creation_options={"compression_level": 9},  # Set high compression level (1-9, 9 being the highest)
            # compression_level=9,  # Set high compression level (only for Deflate)
            zlevel=9,  # Set the Deflate compression level (1-9, 9 being the highest)

            # crs=geo.projection.to_proj4(),
    )
    #we might want to fill in the gaps. useful sometimes...
    if fill:
        from rasterio.fill import fillnodata
